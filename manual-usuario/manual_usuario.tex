%----------------------------------------------------------------------------------------
% Manual de Usuario - Sistema de Firma Electrónica Documenso
...
\usepackage{graphicx}
\graphicspath{{../figuras/apendiceD/}{../figuras/}}
...
\pagestyle{fancy}
\fancyhf{}
\fancyhead[L]{\small Sistema de Firma Electrónica Documenso --- FACET / UNT}
...
\begin{titlepage}
    ...
    \begin{minipage}[c]{0.45\textwidth}
        \centering
        \includegraphics[height=2.8cm]{unt.png}
    \end{minipage}
    \hfill
    \begin{minipage}[c]{0.50\textwidth}
        \centering
        \includegraphics[height=2.8cm]{facet.jpg}
    \end{minipage}
    \par
    \vspace{2.5cm}
    \hrule\vspace{0.4cm}
    {\Huge\bfseries Manual de Usuario}\par\vspace{0.5cm}
    {\LARGE Sistema de Firma Electrónica\\[0.3em] de Documentos}\par
    \vspace{0.4cm}\hrule
    \vspace{1.5cm}
    {\large Plataforma Documenso --- Uso Institucional}\par
    \vspace{3cm}
    {\small Este documento describe el flujo de trabajo para el personal docente,\\
    administrativo y alumnos de la FACET que utilizan la plataforma\\
    de firma electrónica institucional.}\par
    \vfill
    {\small Mayo de 2026}
\end{titlepage}
...
Este manual está dirigido al personal docente, administrativo y a los alumnos de la FACET para el uso cotidiano de la plataforma de firma electrónica Documenso. Se describen los pasos necesarios para firmar documentos institucionales dentro del contexto organizacional correcto. Para funcionalidades avanzadas ---plantillas, campos especiales, configuración de equipos--- se recomienda consultar la documentación oficial del proyecto, disponible en \url{https://docs.documenso.com/docs/users}.
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{selector-org-facet.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{selector-team.png}
...
\label{sec:CargaDocumento}

\subsection{Carga del Archivo}
\label{sec:CargaArchivo}

Desde el panel de documentos del equipo, utilizar el botón \textbf{Cargar Documento} para subir el archivo PDF a firmar (Figura~\ref{fig:apD_cargar_doc}). El sistema presenta dos opciones: se debe seleccionar \textbf{Cargar Documento} e ignorar la opción \textbf{Documento (Legado)}. El tamaño máximo admitido es de 50\,MB y el formato aceptado es exclusivamente PDF.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{boton-cargar-documento.png}
    \caption[Opciones de carga de documento]{Opciones de carga. Seleccionar siempre \textbf{Cargar Documento}; la opción \textbf{Documento (Legado)} no debe utilizarse.}
...
\begin{enumerate}[nosep]
    ...
    \item \textbf{Enlace directo:} se genera un enlace de firma sin enviar correo. Esta modalidad es útil para distribuir el documento por WhatsApp, mensajería interna u otros canales.
\end{enumerate}
...
\begin{figure}[H]
    ...
    \begin{subfigure}[b]{0.45\textwidth}
        \centering
        \includegraphics[width=\textwidth]{envio-modalidades-mail.png}
    ...
    \begin{subfigure}[b]{0.45\textwidth}
        \centering
        \includegraphics[width=\textwidth]{envio-modalidades-link.png}
        \caption{Generación de enlace directo de firma.}
        \label{fig:apD_envio_link}
    \end{subfigure}
    \caption[Modalidades de envío]{Pantalla de envío del documento. (a)~Distribución por correo electrónico. (b)~Generación de enlace directo de firma.}
    \label{fig:apD_envio}
\end{figure}

Tras el envío, el documento pasa del estado \textbf{Borrador} a \textbf{Pendiente}.
...
Si un destinatario no ha actuado sobre el documento, se le puede reenviar un recordatorio desde el menú de acciones (tres puntos) del documento en estado \textbf{Pendiente}.
...
\label{sec:Destinatario}

El destinatario accede al documento a través del enlace recibido por correo o por enlace directo. Al abrirlo, visualiza el PDF con sus campos asignados resaltados. Para los campos de firma, puede optar por dibujar la firma manualmente, escribir su nombre y seleccionar un estilo tipográfico, o cargar una imagen de su firma. Una vez completados todos los campos obligatorios, confirma la acción.

\medskip
\noindent\textbf{Nota aclaratoria sobre la validación del certificado:} al abrir el documento firmado en Adobe Acrobat, es posible que el lector indique que la validez del certificado es desconocida. Este comportamiento es normal y esperado, ya que la plataforma utiliza un certificado criptográfico emitido internamente por la FACET. La integridad del documento está asegurada y el sello es plenamente válido para el uso institucional interno.
...
\begin{enumerate}[nosep]
    ...
    \item Enviar por correo electrónico o generar un enlace directo.
    \item Consultar el estado desde el panel de documentos.
\end{enumerate}

Para funcionalidades adicionales como la creación de plantillas reutilizables, configuración o uso de campos distintos al de firma, o la gestión de equipos, consultar la documentación oficial de Documenso disponible en \url{https://docs.documenso.com/docs/users}.
