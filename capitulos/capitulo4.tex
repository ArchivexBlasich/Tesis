...
Las métricas de consumo de CPU y RAM se registraron por contenedor mediante Netdata. Los valores se contrastan a continuación con la capacidad auditada del LXC: 8~vCPUs y 8~GiB de RAM (Subsección~\ref{sec:infraestructura_base}).
...
\begin{figure}[!htbp]
    ...
    \caption[Consumo de RAM en baseline: cuatro contenedores del \textit{stack}]{Consumo de RAM (en MiB) de los contenedores del \textit{stack} durante el período de observación en baseline. Documenso: \(\approx\) 351~MiB; Browserless: \(\approx\) 308~MiB; PostgreSQL: \(\approx\) 76~MiB; \textit{proxy} Socat: \(\approx\) 2,2~MiB. Total del \textit{stack}: \(\approx\) 737~MiB sobre 8~GiB asignados al LXC.}
    \label{fig:baseline_ram}
\end{figure}

\FloatBarrier

El consumo total del \textit{stack} en reposo fue de aproximadamente 737~MiB, equivalente al 9\,\% de la capacidad del LXC. Documenso concentró la mayor parte de ese consumo (351~MiB), seguido de Browserless (308~MiB). PostgreSQL y Socat aportaron conjuntamente menos de 80~MiB.
...
\begin{figure}[!htbp]
    ...
    \caption[Consumo de CPU en condición de reposo: los cuatro contenedores]{Uso de CPU (\% del límite del cgroup) durante el período de observación. Picos máximos: Documenso 7,8\,\%; Browserless 2,5\,\%; PostgreSQL 1,3\,\%; \textit{proxy} Socat 0,5\,\%.}
...
\begin{figure}[!htbp]
    ...
    \end{subfigure}
    \caption[CPU y RAM durante el sellado de un documento (1 firma)]{Consumo de CPU y RAM del \textit{stack} durante el sellado de un documento. La actividad se concentró en un único instante (22:11:32--22:11:34 UTC): Browserless alcanzó un pico máximo de 104,5\,\% de CPU y 707,7~MiB de RAM antes de liberar ambos recursos al concluir el job.}
    \label{fig:carga_1_firma}
\end{figure}

\FloatBarrier

En CPU, el evento se manifestó como un pico aislado de 2~segundos de duración: Browserless alcanzó un máximo de 104,5\,\% mientras Documenso no superó el 19\,\%. El resto del \textit{stack} permaneció en valores de baseline. En RAM, Browserless escaló abruptamente desde 310~MiB hasta 707,7~MiB (+397~MiB) para instanciar el proceso de Chrome encargado del renderizado. Al concluir el job, el proceso fue destruido y el sistema operativo recuperó la memoria; ambas curvas retornaron a sus valores de baseline en el siguiente intervalo de muestreo. El impacto sobre Documenso (+30~MiB) y PostgreSQL (+13~MiB) resultó marginal.
...
En este ensayo, el bloque de actividad se extendió entre las 17:27:40 y las 17:29:20 UTC, aproximadamente dos minutos: Browserless procesó los cinco jobs en forma secuencial, lo que explica la forma escalonada de la curva de RAM (Figura~\ref{fig:carga_5firmas_ram}), donde cada peldaño corresponde al lanzamiento de un proceso de Chrome adicional. El pico máximo de RAM de Browserless fue de 1\,331~MiB. Al completarse el último job, la curva cayó a los 298~MiB de baseline en un único intervalo de muestreo.
...
\begin{table}[!htbp]
    ...
    \begin{tabular}{l r r r}
        ...
        \quad Browserless   & 2,5\,\%    & 104,5\,\%              & \(\approx\) 13\,\% (\(\approx\) 1~vCPU) \\
        \quad Documenso     & 7,77\,\%   & \(\approx\) 20\,\%   & \(\approx\) 2,5\,\% (\(\approx\) 0,2~vCPU) \\
        \midrule
        \multicolumn{4}{l}{\textbf{Uso de RAM} (capacidad total: 8\,192~MiB)} \\
        \quad Browserless             & \(\approx\) 308~MiB  & \(\approx\) 1\,331~MiB & \(\approx\) 16\,\% \\
        \quad \textbf{\textit{Stack} total}    & \textbf{\(\approx\) 737~MiB (9\,\%)} & \textbf{\(\approx\) 1\,826~MiB} & \textbf{\(\approx\) 22\,\%} \\
        \bottomrule
    \end{tabular}
\end{table}
...
El recurso con mayor variación bajo carga fue la RAM de Browserless, que se multiplicó por 4,3 respecto al baseline. Aun en el peor caso registrado, el \textit{stack} consumió el 22\,\% de la RAM del LXC, lo que dejó un margen de aproximadamente 6\,366~MiB (\(\approx\) 6,2~GiB) disponibles. La CPU no representó un cuello de botella en ninguno de los dos ensayos.
